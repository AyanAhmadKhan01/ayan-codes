...
export default function PostForm() {
    ...
    const handleSaveDraft = () => {
        const blogData = {
            ...
            comments: editingBlog?.comments || 0,
            readTime: Math.ceil(content.replace(/<[^>]*>/g, '').split(' ').length / 200),
            featured: editingBlog?.featured || false
        }

        if (editingBlog) {
            setBlogs(prev => prev.map(blog => blog.id === editingBlog.id ? blogData : blog))
        } else {
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-muted-foreground text-lg">
                            {editingBlog ? 'Update your blog post' : 'Write and publish your blog post'}
                        </p>
                    </div>
                    <Button
                        variant="ghost"
                            ...
                            <div className="border border-border/20 bg-background/50 rounded-none overflow-hidden">
                                {useSimpleEditor ? (
                                    <SimpleEditor
                                        ...
                                        onImageInsert={(url) => console.log('Image inserted:', url)}
                                    />
                                ) : (
                                    <QuillEditor
                                        ref={quillRef}
                        ...
                        <div className="border border-border/20 bg-card/50 backdrop-blur-sm p-6">
                            ...
                            <div className="space-y-3">
                                <div className="flex gap-2">
                                    ...
                                    />
                                    <Button onClick={handleAddTag} size="sm" className="rounded-none">
                                        Add
                                    </Button>
                                </div>
